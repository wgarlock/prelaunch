...
Example usage for ``prequ compile-in``
--------------------------------------
...
Now, run ``prequ compile-in requirements.in``:

.. code::

   $ prequ compile-in requirements.in
...
To update all packages, periodically re-run ``prequ compile-in --upgrade``.
...
.. code::
   ...
   prequ compile-in --upgrade-package flask
     # only update the flask package
   prequ compile-in --upgrade-package flask --upgrade-package requests
     # update both the flask and requests packages
   prequ compile-in -P flask -P requests==2.0.0
