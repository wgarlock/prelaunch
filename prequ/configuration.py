...
class PrequConfiguration(object):
    ...
    fields = [
        ...
        ('options.app_name', text),
        ('options.github_actions', [text]),
        ('github_actions_directory', text)
    ]
    ...
    def __init__(self, requirement_sets, **kwargs):
        ...
        self.github_actions = kwargs.pop('github_actions', [])
        self.github_actions_directory = kwargs.pop('github_actions_directory', '.github/workflows')
    ...
    def get_output_file_for_nonreq(self, label):
        """
        Get output file name for a requirement set.
